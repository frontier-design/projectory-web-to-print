...
app.post('/debug-html', async (req, res) => {
  try {
    ...
    const cssContent = await fs.readFile(path.join(__dirname, '..', 'styles.css'), 'utf-8');
    const orangeSvg = await fs.readFile(path.join(assetsPath, 'orange.svg'), 'base64');
    const blueSvg = await fs.readFile(path.join(assetsPath, 'blue.svg'), 'base64');
    ...
    const cssWithFonts = cssContent
      .replace(/url\('\.\/assets\/fonts\/FoundersGrotesk-Regular\.otf'\)/g, 
        `url(data:font/opentype;base64,${founderGroteskRegular})`)
      .replace(/url\('\.\/assets\/fonts\/FoundersGrotesk-Medium\.otf'\)/g, 
        `url(data:font/opentype;base64,${founderGroteskMedium})`)
      .replace(/url\('\.\/assets\/fonts\/PermanentMarker-Regular\.ttf'\)/g, 
...
function generateHTML(items, cssContent, orangeSvg, blueSvg) {
  const printSurfaces = items.map(item => `
...
app.post('/generate-pdfs', async (req, res) => {
  try {
    ...
    const cssWithFonts = cssContent
      .replace(/url\('\.\/assets\/fonts\/FoundersGrotesk-Regular\.otf'\)\s*format\('opentype'\)/g, 
        `url(data:font/opentype;base64,${founderGroteskRegular}) format('opentype')`)
      .replace(/url\('\.\/assets\/fonts\/FoundersGrotesk-RegularItalic\.otf'\)\s*format\('opentype'\)/g, 
        `url(data:font/opentype;base64,${founderGroteskRegular}) format('opentype')`)
      .replace(/url\('\.\/assets\/fonts\/FoundersGrotesk-Medium\.otf'\)\s*format\('opentype'\)/g, 
        `url(data:font/opentype;base64,${founderGroteskMedium}) format('opentype')`)
      .replace(/url\('\.\/assets\/fonts\/PermanentMarker-Regular\.ttf'\)\s*format\('truetype'\)/g, 
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      console.log(`Generating batch ${batchIndex + 1}/${totalBatches} (${batchItems.length} items)...`);
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=comboconvo-pdfs.zip');
    res.send(zipBuffer);
